...
export default function SectionNavigationItem({
  ...
  const handleClick = () => {
    if (isClickable) {
      ...
      url.searchParams.set("section", section.id.toString());
    ...
    <div className="relative">
        ...
        className={`
          ...
          ${
            status === "current"
              ...
              : "bg-slate-50/50 border border-slate-100 hover:bg-white hover:border-slate-200 hover:shadow-sm"
          }
      ...
      >
        ...
        {status === "completed" && isClickable && (
          <svg
            className="w-3 h-3 lg:w-4 lg:h-4 text-slate-400 group-hover:text-slate-600 transition-colors duration-200"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
